...
export default function TenantRegisterScreen({ navigation }) {
  ...
  const pickImage = async () => {
    ...
    if (!result.canceled) {
      ...
      if (sizeKB > 100) {
        Alert.alert("Image Too Large", "Image must be less than 100KB");
        return;
      }
  ...
  const handleRegister = async () => {
    ...
    console.log("Register API JSON:", JSON.stringify(registerPayload, null, 2));
    ...
    try {
      ...
      const response = await fetch(
        "http://192.168.1.32:8000/register/tenent/",
        {
          method: "POST",
          body: formData,
        },
